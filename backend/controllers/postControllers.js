...
// Check if post exists and if it's password protected
export const checkPost = async (req, res) => {
    ...
    try {
        ...
        // Only return whether the post is password protected or not
        res.status(200).json({
            isPasswordProtected: !!post.password
        });
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
...
// Verify password and return post if correct
export const verifyPostPassword = async (req, res) => {
    const { id } = req.params;
    const { password } = req.body;

    console.log('Verifying password for post:', id);
    ...
    try {
        ...
        // Password is correct, return the post
        res.status(200).json(post);
    } catch (err) {
